...
exports.register = async (req, res) => {
  ...
  try {
    ...
    const userData = new UserData({ name, email, profilePhoto: fileUrl, mobileNumber });
    console.log(userData);
    await userData.save();

    // Sign JWT for userData details
    const token = jwt.sign(
      { _id: userData._id, name: userData.name, email: userData.email, profilePhoto: userData.profilePhoto, mobileNumber: userData.mobileNumber },
      'secret123',
      { expiresIn: '30m' }
    );
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ _id: userData._id, name: userData.name, email: userData.email, profilePhoto: userData.profilePhoto, mobileNumber: userData.mobileNumber }, 'secret123', { expiresIn: '300m' });
